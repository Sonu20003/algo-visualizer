...
function generateRandomArray(blockCount) {
    const array = [];
...
function generateRandomSearchValue(array) {
    const index = Math.floor(Math.random() * array.length);
    const value = array[index];
    return value;
}

function createArraySearch(blockCount , array){
    while(container.firstChild){
        container.removeChild(container.firstChild);
    }
    for(let i=0; i<blockCount; i++){
        const block = document.createElement(`span`);
        //block.classList.add("newSpan");
...
async function linearSearch(){
    ...
    let steps=0;
    const array = generateRandomArray(blockCount);
    createArraySearch(blockCount, array);
    ...
    textarea.classList.add("myTextarea");
    let searchElement = generateRandomSearchValue(array);
    value.textContent=searchElement;
    const index = document.createElement(`div`);
    ...
    for(let i=0; i<blockCount; i++){
        ...
        blocks[i].style.borderColor = `#e74c3c`;;
        ...
        if(searchElement == array[i]){
            textarea.textContent = `Element found at index: ${i}\nTotal Iterations: #${steps}`;
            blocks[i].classList.add("glow-border");
            block.classList.add("glow-border");
            found = false; 
            break;
        }
        blocks[i].style.borderColor = ogColor;
    }
    if(found){
        textarea.textContent = `Element not found !!\nTotal Iterations: #${steps}`;  
    }
}
export {linearSearch};
//Binary Search  

function generateSortedArray(blockCount) {
    const array = [];
    for (let i = 0; i < blockCount; i++) {
        const randomValue = Math.floor(Math.random() * 100);
        insertIntoSortedArray(array, randomValue);
    }
    return array;
}
...
async function binarySearch(){
    ...
    const array = generateSortedArray(blockCount);
    createArraySearch(blockCount, array);
    const block = document.createElement(`span`);
    const value = document.createElement(`div`);
    const textarea = document.createElement(`textarea`);
    ...
    textarea.classList.add("myTextarea");
    let target = generateRandomSearchValue(array);
    ...
    while (start <= end) {
        blocks[mid].style.borderColor=`#e74c3c`;
        await new Promise(resolve => setTimeout(resolve, delay+500));
        blocks[mid].style.borderColor=`#4d2925`;
        steps++;
        
        if (target == array[mid]){
            found = true;
            textarea.textContent = `Element found at index: ${mid}\nTotal Iterations: #${steps}`;
            blocks[mid].classList.add("glow-border");
            block.classList.add("glow-border");
            found = false; 
            break;
            ...
            await disable(blocks, start, mid);
            start = mid + 1;
        } 
        else {
            await disable(blocks, mid, end);
            end = mid - 1;
        }
        mid = Math.floor((start + end) / 2);
    }
...
async function disable(blocks, start, end){
    let i=start;
    while(i<=end){
